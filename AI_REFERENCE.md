...
- `verilog_yaml_bridge`: maintains the descriptor workflow (`sand_module`/`sand_header` ↔ machine snapshots). `export` can refresh machine files; day-to-day edits happen in `rtl.yaml`, then `restore` rebuilds Verilog (`build/rtl`). Streaming blocks remain `verilog_module_fallback` (stores original text) until full SystemVerilog coverage lands.
