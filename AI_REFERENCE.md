...
- `verilog_yaml_bridge`: maintains the descriptor workflow (`sand_module`/`sand_header` ↔ machine snapshots). `export` now emits human-friendly descriptors (parameters/ports/logic summaries) for modules already tracked as `kind: sand_module`—notably `rtl/circuits/*`—while copying the canonical RTL into `rtl.yaml/machine/` and preserving manual notes/fanout metadata. Day-to-day edits happen in `rtl.yaml`, then `restore` rebuilds Verilog (`build/rtl`). Streaming blocks remain `verilog_module_fallback` (stores original text) until full SystemVerilog coverage lands.
