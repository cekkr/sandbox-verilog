...
def run_example(
    ...
    config_header = build_dir / "neural_edge_slice_config.vh"

    sand_configurator.write_neural_edge_header(params, config_header)
...
def main() -> None:
    ...
    parser.add_argument("--config", type=pathlib.Path,
                        help="Optional YAML/JSON file describing the pipeline")
    ...
    if args.window_width is not None:
        params = replace(params, window_w=args.window_width)
    if args.window_height is not None:
        params = replace(params, window_h=args.window_height)
    if args.pattern is not None:
        pattern = args.pattern.lower()
        params = replace(
            params,
            pattern=pattern,
            pattern_id=sand_configurator.PATTERN_TO_ID[pattern],
        )
    if args.edge_gain is not None:
        params = replace(params, edge_gain_pct=int(round(args.edge_gain * 1000.0)))
    if args.raw_gain is not None:
        params = replace(params, raw_gain_pct=int(round(args.raw_gain * 1000.0)))
    if args.bias is not None:
        params = replace(params, bias_pct=int(round(args.bias * 1000.0)))
    if args.threshold is not None:
        params = replace(params, threshold_pct=int(round(args.threshold * 1000.0)))
